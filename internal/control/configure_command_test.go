...
func TestRunConfigureCommandWritesNodeConfig(t *testing.T) {
	...
	err := RunConfigureCommand([]string{"--panel-url", server.URL, "--token", "configure-token", "--node", "encoder-recorder-01", "--config", configPath}, ServiceType, &out)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(string(body), `id: "encoder-recorder-01"`) || strings.Contains(string(body), "configure-token") {
	...
	if !strings.Contains(out.String(), "configure succeeded: wrote") || !strings.Contains(out.String(), "encoder-recorder-01") {
...
func TestRunConfigureCommandRejectsNodeTypeMismatchBeforeWrite(t *testing.T) {
	...
	configPath := filepath.Join(t.TempDir(), "config.yml")
	err := RunConfigureCommand([]string{"--panel-url", server.URL, "--token", "configure-token", "--node", "encoder-recorder-01", "--config", configPath}, ServiceType, nil)
